	...
	<div class="container">
		...
		<div class="grid grid-cols-12 gap-6">
			<div class="col-span-12 mx-auto w-1/2">
				<div class="grid grid-cols-12 gap-6 items-center mb-6 !hidden" id="search_container">
					<!-- Page Header --> 
					...
					<div class="lg:col-span-12 col-span-12">
						<div class="inline-flex !w-full companies-search-input">
							<select id="startLocation" class="form-control cursor-not-allowed" disabled name="startLocation"><option value="{{ $visite->visiteur->agence->ville->id_ville }}" selected>{{ $visite->visiteur->agence->ville->nom }}</option></select>
							<select id="endLocation" class="form-control cursor-not-allowed" disabled name="endLocation"><option value="{{ $visite->praticien->ville->id_ville }}" selected>{{ $visite->praticien->ville->nom }}</option></select>
							<input type="text" disabled class="form-control cursor-not-allowed" id="DateDepart" name="DateDepart" placeholder="Date de départ" readonly="readonly" value="{{ $visite->created_at->format('d/m/Y') }}">
						</div>
					</div>
				</div>
				<div class="mt-28 w-1/2 mx-auto text-center" id="loading_annonce">
					<img src="{{ asset('assets/img/you_want_to_carpool.gif') }}">
                    <div class="mt-12 font-bold text-center text-xl">
                        <span id="text-1">Nous rechercherons des offres de covoiturage pour vous...</span>
                    </div>
				</div>
                <div class="mt-28 w-1/2 mx-auto text-center !hidden" id="no_annonce_found">
					<img src="{{ asset('assets/img/no_annonce_found.png') }}" class="mx-auto justify-center w-1/2">
                    <div class="mt-12 font-bold text-center text-xl">
                        <span id="text-1">Nous n'avons trouver aucune offre de covoiturage pour vous<br>Veuillez patientez vous seriez redériger</span>
                    </div>
				</div>
				<div class="box custom-box !hidden" id="annonce_metric">
					<div class="box-body">
						<div class="sm:flex items-center">
							<h5 class="font-semibold mb-0 flex-grow">
                                <b id="covoiturage_count"></b>
                                <span class="font-normal text-[1.125rem]">covoiturage(s) correspond à votre recherche.</span>
                            </h5>
						</div>
					</div>
				</div>
				<div id="annonces"></div>
                <div class="w-full !hidden" id="je_ne_veux_pas_covoiturer">
                    <a href="{{ route('visite.create_confirmed', ['id_visite' => $visite->id_visite]) }}">
                        <button class="ti-btn ti-btn-danger w-full ti-btn-wave" type="button">Je ne veux pas covoiturer !</button>
                    </a>
                </div>
			</div>
		</div>
	       ...
	       .then((responseJson) => {
                window.setTimeout(function(){
                    ...
                    if(responseJson.length < 1){
                        $('#no_annonce_found').removeClass('!hidden');
                        window.setTimeout(function(){
                            window.location.href = '{{ route('visite.create_confirmed', ['id_visite' => $visite->id_visite]) }}';
                        }, 1500);
                        return false;
                    }
                    ...
                    $('#je_ne_veux_pas_covoiturer').removeClass('!hidden');
